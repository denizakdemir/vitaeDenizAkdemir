...
stats_publications <- all_publications[grepl(paste(stats_keywords, collapse="|"), 
                                            tolower(all_publications$title), 
                                            fixed = FALSE)]

# Display statistics-focused publications (only if found, otherwise show first 10)
if(length(stats_publications) > 0) {
  stats_publications
} else {
